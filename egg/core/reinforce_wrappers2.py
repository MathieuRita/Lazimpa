...
class SenderReceiverRnnReinforce(nn.Module):
    ...
    def forward(self, sender_input, labels, receiver_input=None):
        ...
        # Loss de similitude des messages
        input_ids=torch.where((sender_input == 1.))[1]
        input_to_keep=[]
        for i in range(100):
          if torch.where((input_ids== i))[0].nelement()>0:
            input_to_keep.append(torch.where((input_ids== i))[0][0])

        slice_message=message[input_to_keep[0]].unsqueeze(0)

        for i in range(1,len(input_to_keep)):
          nmes=message[input_to_keep[i]].unsqueeze(0)
          slice_message=torch.cat((slice_message,nmes),0)
        
        table=slice_message[:].sum(axis=1)
        table=table.repeat((table.size()[0],1))
        table0=table
        for i in range(table.size()[0]):
          table[i]=table[i]-table0[i,i]
        
        ns=table.size()[0]
        pen=1*(table==0)
        pen=pen.sum(axis=1).sum(axis=0)/2

        loss.add_(-0.3*(pen/ns))
        ###### FIN AJOUT
